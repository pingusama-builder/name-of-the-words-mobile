...
export default function WordDetail({ word, onClose }: WordDetailProps) {
  ...
  const hasMatch = word.pairedWord;
  const matchScore = hasMatch
    ? Math.round(
        ((word.ratingEssence || 0) + (word.ratingBeauty || 0) + (word.ratingSubtlety || 0)) / 36 * 100
      )
    : 0;
  ...
  const handleDelete = () => {
    setIsDeleting(true);
    // play dissolve, then actually delete
    setTimeout(() => {
      ...
      >
        ...
        <div className="px-6">
          ...
          <div className="flex items-start justify-between mb-1">
            <h2 className="font-serif text-xl text-foreground">{word.word}</h2>
          ...
          {/* Date */}
          <p className="text-xs text-muted-foreground/50 mb-4">{word.dateAdded}</p>
          ...
          {word.pairedWord && (
            <div className="mb-5 p-3 rounded-xl border border-primary/15 bg-primary/5">
              ...
              <div className="flex items-center gap-3">
                <span className="text-base text-foreground">{word.word}</span>
                ...
                <span className="text-base text-foreground">{word.pairedWord}</span>
              </div>
              {word.pairedMeaning && (
                <p className="text-xs text-muted-foreground/60 mt-1.5">{word.pairedMeaning}</p>
              )}
              {/* Match meter */}
          ...
          {word.context && (
            <div className="mb-5">
              ...
              <p className="text-sm text-foreground/60 italic">"{word.context}"</p>
            </div>
          )}

          {/* Rating dials — read-only display */}
          <div className="flex justify-around mb-5 py-2">
            <CircularDial value={word.ratingEssence || 0} onChange={() => {}} label="Essence" color="#4fb8a3" size={72} />
            <CircularDial value={word.ratingBeauty || 0} onChange={() => {}} label="Beauty" color="#9b7fd4" size={72} />
            <CircularDial value={word.ratingSubtlety || 0} onChange={() => {}} label="Subtlety" color="#d4a34f" size={72} />
          </div>
